...
class CreateCorporateAssetsScrappedTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('corporate_assets_scrapped', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('num')->length(11)->comment('报废单号');
            $table->timestamp('scrapped_at')->nullable(true)->comment('报废时间');
            $table->integer('apply_user_id')->length(11)->comment('实际申请人ID');
            $table->integer('user_id')->length(11)->comment('报废操作人ID');
            $table->text('remarks')->comment('备注');
            $table->integer('entry_id')->length(11)->comment('工作流ID');
            $table->timestamps();
            $table->softDeletes();//技术
        });
        DB::statement("ALTER TABLE `corporate_assets_scrapped` comment '公司资产报废'");
    }
    ...
    {
        Schema::dropIfExists('corporate_assets_scrapped');
    }
}
