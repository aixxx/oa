...
class CreateAttendanceApiStaffTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('attendance_api_staff', function (Blueprint $table) {
            ...
            $table->integer('user_id')->length(11)->comment('员工ID');
            $table->tinyInteger('is_attendance')->length(1)->comment('是否需要参与考勤 1-需要 2-不需要')->default(0);
            $table->timestamp('created_at')->nullable()->comment('创建时间');
            $table->timestamp('updated_at')->nullable()->comment('更新时间');
            $table->timestamp('deleted_at')->nullable()->comment('删除时间');
            $table->index('deleted_at');
            $table->comment = '参与考勤人员';
        });
    }
    ...
    {
        Schema::dropIfExists('attendance_api_staff');
    }
}
