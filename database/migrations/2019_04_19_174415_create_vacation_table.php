...
class CreateVacationTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('company_vacation', function (Blueprint $table) {
            ...
            $table->tinyInteger('is_balance')->nullable()->default(1)->comment('是否启用余额1、开0、关');
            $table->integer('balance_type')->nullable()->default(1)
                ->comment('余额发放形式1、每年自动固定发放天数2、按照入职时间自动发放3、加班时长自动计入余额');
            ...
            $table->tinyInteger('is_add_expire')->nullable()->default(0)->comment('是否支持延长有效期1、是0、否');
    ...
    {
        Schema::dropIfExists('company_vacation');
    }
}
