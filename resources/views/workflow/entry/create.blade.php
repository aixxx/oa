    ...
    <section class="page-content container-fluid">
        <div class="card">
            <div class="card-body">
                <form id="order-form" action="" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" name="entry_id" @if(empty($entry)) value=""
                           @else value="{{$entry->id}}" @endif>
                    ...
                    <button type="button" class="btn btn-primary pull-right" id="save-draft-btn"
                            data-href="{{route('workflow.entry.store')}}">保存草稿
                    </button>
                    <button type="button" class="btn btn-success pull-right" id="submit-apply"
                            data-href="{{route('workflow.entry.store')}}"
                            style="margin-right: 10px;">提交申请
                    </button>
                </form>
            </div>
        </div>
        @if($flow->introduction)
            <div class="card">
                <div class="card-body">
                    <div>
                        @foreach(explode(PHP_EOL, str_replace("\r\n",PHP_EOL,$flow->introduction)) as $introduction)
