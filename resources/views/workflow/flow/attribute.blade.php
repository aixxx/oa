...
<form class="form-horizontal"  method="post" id="flow_attribute" name="flow_attribute" action="{{route('workflow.process.update',['id'=>$process->id])}}">
    <div class="tab-content">
        <div class="tab-pane active" id="attrBasic">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input style="width: 30%;" type="text" id="process_name" name="process_name" placeholder="步骤名称" value="{{$process->process_name}}">
                </div>
            </div>
            ...
            <div class="control-group">
                <label class="control-label" for="process_name">是否参与合并</label>
                <div class="controls">
                    <label class="radio inline">
                        <input type="radio" name="can_merge" @if($process->can_merge==1) checked="checked" @endif value="1">是
                    </label>
                    <label class="radio inline">
                        <input type="radio" name="can_merge" @if($process->can_merge==0) checked="checked" @endif value="0">否
                    </label>
                </div>
            </div>

            <div class="control-group">
                <label class="control-label" for="process_name">审批通过触发事件</label>
                <div class="controls">
                    <input type="text" style="width: 60%;" id="pass_events" name="pass_events" placeholder="事件类型,逗号分隔" value="{{$process->pass_events}}">
                </div>
            </div>

            <div class="control-group">
                <label class="control-label" for="process_name">驳回申请触发事件</label>
                <div class="controls">
                    <input type="text" style="width: 60%;" id="reject_events" name="reject_events" placeholder="事件类型,逗号分隔" value="{{$process->reject_events}}">
                </div>
            </div>
        ...
        <div class="tab-pane" id="attrForm">
            ...
            <table class="table table-condensed table-bordered table-hover" >
                <tr>
                    ...
                    {{--<th style="text-align:center">是否对本节点隐藏</th>--}}
                </tr>
                <tbody>
                ...
                @foreach($fields as $v)
                    <tr>
                        ...
                        {{--<td style="text-align:center"><input type="checkbox" value="{{$v->id}}"></td>--}}
                    </tr>
                @endforeach
                <!-- 这里是表单设计器的字段 end -->
        ...
        <div class="tab-pane" id="attrPower">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <select name="auto_person" id="auto_person_id">
                        ...
                        @foreach ($sys_auditor as $auditor_flag => $auditor_des)
                            <option value="{{$auditor_flag}}" @if($sys==$auditor_flag) selected="selected" @endif>{{$auditor_des}}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <input class="input-xlarge" readonly="readonly" type="hidden" placeholder="选择人员" name="range_emp_text" id="range_emp_text" value="{{$select_emps->implode('name',',')}}">
            <input type="hidden" name="range_dept_ids" id="range_dept_ids" value="{{$select_depts->implode('id',',')}}">
            <input class="input-xlarge" readonly="readonly" type="hidden" placeholder="选择部门" name="range_dept_text" id="range_dept_text" value="{{$select_depts->implode('dept_name',',')}}">
            <input class="input-xlarge" type="hidden" value="{{$depands}}" name="depands" id="depands" placeholder="默认员工归属公司，修改填field_name">
        <!--<div class="control-group">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="hidden" name="range_role_ids" id="range_role_ids" value="{{$select_roles->implode('id', ',')}}">
                    <input class="input-xlarge" readonly="readonly" type="text" placeholder="选择角色" name="range_role_text" id="range_role_text" value="{{$select_roles->implode('role_name', ',')}}"> <a href="javascript:void(0);" class="btn" onclick="superDialog('/workflow/flowlink/auth/role/{{$process->id}}','range_role_text','range_role_ids');">选择</a>
                </div>
            </div>
            <!--<div class="control-group">
                <label class="control-label">公司字段</label>
                <div class="controls">
                    <input class="input-xlarge" type="text" value="{{$depands}}" name="depands" id="depands" placeholder="默认员工归属公司，修改填field_name">
                </div>
            </div>
...
<script type="text/javascript">
    ...
    var get_con_url = "/workflow/process/condition";//获取条件
    ...
    function fnAddLeftParenthesis(id){
        ...
        var current = 0;
        var hasConditionExisted = oObj.options.length > 0; // 已经有条件了
        if(hasConditionExisted){ //检查是否有条件
